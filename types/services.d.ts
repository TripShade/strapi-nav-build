...
export type NavigationServiceName = "common" | "admin" | "client" | "navigation-item";
...
export interface IAdminService {
    ...
    fillFromOtherLocale(payload: {
        source: number;
        target: number;
        auditLog: AuditLogContext;
    }): Promise<Navigation>;
    readNavigationItemFromLocale(payload: {
        source: number;
        target: number;
        path: string;
    }): Promise<Partial<Pick<NotVoid<Navigation["items"]>[number], "path" | "related" | "type" | "uiRouterKey" | "title" | "externalPath">>>;
}
...
export interface IClientService {
    ...
    renderRFR: (input: {
        ...
        parent?: Id | null;
        parentNavItem?: RFRNavItem | null;
        contentTypes: string[];
        enabledCustomFieldsNames: string[];
    }) => ToBeFixed;
    renderRFRNav: (item: NavigationItem) => RFRNavItem;
    renderRFRPage: (item: NavigationItem & {
        related: ToBeFixed;
    }, parent: Id | null, enabledCustomFieldsNames: string[]) => ToBeFixed;
    ...
    renderType: (input: {
        type: RenderType;
        criteria: ToBeFixed;
        itemCriteria: ToBeFixed;
        filter: ToBeFixed;
        rootPath: string | null;
        wrapRelated?: boolean;
        populate?: PopulateQueryParam;
    } & I18nQueryParams) => ToBeFixed;
}
